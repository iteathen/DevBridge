...
The integrated Stage 8 source is `b3b5801c8e54cc8c15d6c2a5ff0867a1befb864f` (PR #510). The ejected-media fix passed all four Ubuntu/Windows [CI jobs](https://github.com/iteathen/DevBridge/actions/runs/34027916924).

- **Ubuntu construction (#197):** recipe v18/output v14 completed real installation, qualification, sanitization and image capture through the installed entry. The qualification reports Ubuntu 26.04, GCC 15.2, CMake 4.2.3, Node 22.22.1 and successful CMake/CTest. Automatic DVD ejection is reconciled only after exact VM/disk/slot ownership checks. The originating GitHub task path still needs proof.
...
- **Windows construction:** official Windows 11 Pro media is selected and activation deferred. Use an explicitly selected empty local construction directory when the installation drive cannot hold the temporary disk/media and final image together; preserve reserve checks on both volumes. Then qualify the real compiler, CMake and CTest. An accepted configuration or mocked provider does not establish readiness.
