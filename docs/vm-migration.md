...
Stage 8 setup/reconfiguration must recognize old repository-owned environments as migration candidates and must not silently count them as ready profile environments.
...
`all` in repository selection means all eligible repository workspaces. Compatibility CLI flags such as `--environment`, `--all-environments`, and `--no-environments`, where retained during migration, must mean workspace selection rather than repository-owned VM lifecycle.
