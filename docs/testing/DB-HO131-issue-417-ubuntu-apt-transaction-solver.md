...
- APT process rejection reports bounded, control-sanitized exit/signal/diagnostic evidence so hosted and release failures remain actionable without returning transaction authority.
...
The second hosted run `33652253489` then reached the first real simulation but the generic adapter error did not yet expose the exact APT diagnostic. A compatibility audit found that Ubuntu 24.04 supplies APT 2.8.3: its `apt-get` interface documents `--snapshot`, `--no-remove`, and simulation, but does not define the newer `--no-list-columns` presentation switch. That unnecessary presentation flag was removed while exact snapshot and no-removal arguments remained; a focused argument assertion prevents reintroducing it. This was a compatibility correction, not yet the classification of the observed failure.

The third hosted run `33652692209` still failed at the first real simulation, while the adapter's generic process error concealed the bounded APT diagnostic. The adapter then preserved fail-closed behavior but began reporting bounded, control-sanitized exit/signal/stderr evidence. This is an operational-correctness correction: it exposes no transaction authority and makes hosted classification exact rather than inferential.

The fourth hosted run `33653125099` reported the exact remaining condition: APT exited zero but warned that the synthetic repository's `Release` file had an invalid `Date` entry because the fixture omitted that field. The solver correctly rejected the warning. The correction belongs solely to the synthetic metadata producer: add a fixed RFC-formatted `Date` matching the selected fixture snapshot, then require a fresh all-four run.
