...
The third hosted run `33652692209` still failed at the first real simulation, while the adapter's generic process error concealed the bounded APT diagnostic. The adapter now preserves fail-closed behavior but reports bounded, control-sanitized exit/signal/stderr evidence. This is an operational-correctness correction: it exposes no transaction authority and makes the next hosted classification exact rather than inferential.
