...
The first hosted full-matrix attempt at exact head `51063e6d0500ae745708b32e657afe070e893daf` exposed one Ubuntu failure in the new unknown-membership rejection test. The compatibility assertion did not run because cleanup of the disposable source checkout received `ENOTEMPTY` beneath its `.git` directory. Git documents that `fetch` runs automatic maintenance by default, while automatic maintenance may detach and continue in the background. That lifetime is incompatible with a source-channel adapter that owns and removes the checkout immediately after component publication. The adapter now passes `--no-auto-maintenance` on its fixed shallow fetch; maintenance has no retained value for this disposable checkout, and the source child keeps ownership of every process that can mutate its staging root. A child-boundary regression pins the fixed fetch argument. Component cleanup retries were not widened, the compatibility verifier was not weakened, and no test-only source bypass was added.

Primary sources: [Git fetch automatic maintenance](https://git-scm.com/docs/git-fetch.html#Documentation/git-fetch.txt---no-auto-maintenance) and [Git maintenance detachment](https://git-scm.com/docs/git-maintenance#Documentation/git-maintenance.txt-maintenanceautoDetach).
