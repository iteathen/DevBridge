...
The fifth complete run `33580625567` passed both Ubuntu jobs and Windows smoke. Windows full cleared the configuration-path exception and reached strict bound-input validation, where it returned `InvalidDataException`. Every bound path in the direct helper test was fixture-owned except `process.execPath`, supplied by the hosted setup-node tool cache; the helper intentionally rejects filesystem indirection in every executable path component. Keep that production protection strict. Give the test a fixture-owned `node.exe` byte artifact, while the separate elevation-adapter tests continue to prove the fixed Node command and argv. Focused evidence remains 87/87, preflight remains 2 / 256 / 2 / 206, and cleanup removed all 29 new temporary roots. Require a fresh complete matrix; do not admit hosted tool-cache topology into the broker contract.

The sixth complete run `33581065389` again passed Ubuntu smoke/full and Windows smoke, but Windows full returned the same bounded `InvalidDataException` after the direct helper fixture owned every bound file. That rejects the tool-cache inference; do not guess at another filesystem boundary and do not weaken validation. Add one diagnostic-only seam around input validation: preserve the original exception as the inner cause and expose only a hard-coded stage token such as `stage=deserialize` or `stage=node-path`. Never emit an input value, path, or exception message. A malformed-input process test must prove the token is exact and the fixture root is absent before another complete hosted matrix classifies the owned failure.

The diagnostic-only candidate passes all 87 focused command/setup/elevation tests. Its malformed-input process proof emits the exact `stage=protocol` token, retains `InvalidDataException` only as a bounded inner type, and proves the fixture root is absent from standard error. Repository preflight passes 2 standalone artifacts / 256 syntax files / 2 JSON files / 206 dependency-selected tests; the hosted-equivalent architecture/product/standalone selection passes 36 with one expected Windows symlink-capability skip; and the complete serialized suite passes 2,128 total / 2,107 passed / 21 expected platform skips / zero failures / zero cancellations in 326.2 seconds. Doctor is green, standalone regeneration and diff hygiene pass, and no new disposable elevation roots remain. Require a fresh complete hosted matrix; no setup, UAC, protected-host, provider, VM, guest, or repository-task effect occurred.

The seventh complete run `33582093924` passed Ubuntu smoke/full and Windows smoke. Windows full reported the exact owned boundary `stage=binding-digest`: protocol, shape, identity, managed channel, every physical/reparse path, fixed filenames, runner root/head, and both file-byte digests had already passed. JavaScript binds the exact normalized path strings placed in the input, while the helper unnecessarily recomputed the binding from a second .NET-normalized representation. The elevated child later consumes the exact input fields. Keep every structural and physical validation unchanged, but recompute the digest from those exact published strings so both runtimes bind the same bytes. Add a source contract preventing regression to a second path representation, rerun focused/preflight/hygiene, and require another complete matrix.
