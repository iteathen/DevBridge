...
Fresh run `33631684036` proved that producer admission now works: the dirty/head/origin rejection sequence and CLI pass on hosted Windows. It exposed the same spelling defect one layer later, where materialization compared the checkout's canonical real root to its selected destination as text. Apply the same neutral identity check to checkout evidence and require that the returned root resolve to the existing nonsymbolic selected directory. The end-to-end test varies both source and destination spellings on Windows. No deadline changed; require focused/bounded local proof and another fresh complete hosted matrix.

The final identity correction passes 20/20 focused source/identity tests and bounded preflight at 3 standalone artifacts / 268 syntax files / 2 JSON files / 214 targeted test files. The preceding exact complete suite remains green, and the fresh hosted Windows full job will repeat that complete exact-version coverage with this last directly exercised change. Matching local qualification roots are zero.
