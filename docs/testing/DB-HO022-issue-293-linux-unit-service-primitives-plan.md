...
1. Add a generic definition-reconciliation module with strict closed inputs, bounded UTF-8 definition bytes, strict observation/effect evidence protocols, exact fact-local post-effect observation, true no-op behavior, and restart recovery by observation.
...
`definition-reconciliation.js` now owns only exact definition readiness. It accepts a bounded definition and the four neutral `observe`, `publish`, `refresh`, and `persist` studs. Its observation schema contains only the independent target-relative `stored`, `current`, and `persistent` facts. It rejects malformed or widened observations, requires a strict successful action result, re-observes after every effect, rejects any neighboring fact change, performs no effects for exact readiness, and resumes an interrupted completed effect from observation rather than replaying it. It contains no platform, system-manager, path, service identity, provider, repository, VM, or lifecycle topology.
