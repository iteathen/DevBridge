...
- [Microsoft's Windows 11 download page](https://www.microsoft.com/en-us/software-download/windows11) publishes SHA-256 verification guidance. A measured local ISO is not approved merely because its filename resembles Microsoft media; setup must bind a locally approved official hash/provenance subject.
...
- The bounded discovery pass over `C:\Hyper-V`, Downloads, Desktop, Documents, and OneDrive still finds only the two Ubuntu ISO files under `C:\Hyper-V\Install Media\CUDA-JS`. No Windows ISO is present, so the canary correctly remains unavailable before mutation.
...
1. Place an official owned Windows 11 x64 ISO beneath a locally selected media root and record the Microsoft-published SHA-256 reference.
