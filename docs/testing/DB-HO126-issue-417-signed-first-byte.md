...
## Hosted qualification checkpoint

PR #450 run `33621362660` passed Ubuntu smoke/full and Windows full, but Windows smoke failed in 32 seconds because standalone regeneration found only `first-byte-devbridge.mjs` stale. This was not a timeout or test failure. The two pre-existing generated artifacts pin LF checkout in `.gitattributes`; the new third artifact omitted that publication constraint, so Git checkout transformed its committed LF bytes on Windows before the deterministic builder compared them. Pin only the new generated artifact to LF and add a source assertion for that contract. Keep the compiler, regeneration gate, workflow bounds, and source logic unchanged; require a fresh complete four-job run rather than rerunning only the failed job.
