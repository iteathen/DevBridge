...
- The next live activation exposed a race between the accepted daemon's VM cycle and candidate validation over the same environment lifecycle guard. The supervisor now token-pause-acknowledges the accepted daemon at its cooperative cycle boundary before candidate validation, resumes it after rejection, and sends the stop request only after successful validation. This keeps the last-known-good process available without allowing concurrent lifecycle mutation.
- A later installed canary used two different config paths under one installation home and exposed that the activation journal/runtime-candidate directory were home-wide while supervisor ownership was only daemon-state-directory-wide. The managed supervisor now acquires a home-derived OS endpoint (Windows named pipe or Linux abstract Unix-domain socket) plus an exact PID/token-bound home claim before stopping or validating anything. The endpoint serializes dead-owner reconciliation; the claim also prevents a Windows pipe-instance/platform regression from admitting a second supervisor. Competing alternate-config updaters fail closed.
