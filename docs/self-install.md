...
- when the verifier is absent and setup is already elevated, the owning Windows prerequisite adapter fetches only the exact runtime-pinned official Gpg4win installer with bounded Node networking, verifies the pinned SHA-256 before execution, performs the vendor-supported silent installation, cleans the transient installer, re-discovers `gpgv.exe`, and executes it before claiming readiness;
