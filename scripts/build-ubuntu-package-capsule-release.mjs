...
const MAX_RECIPE_BYTES = 32 * 1024 * 1024;
const OPTIONS = new Set([
  '--recipe', '--destination', '--key-id', '--private-key', '--public-key', '--gpgv', '--keyring', '--chunk-bytes',
]);
...
function parseArgs(argv) {
  if (argv.length % 2 !== 0) throw new TypeError('Ubuntu capsule release arguments require option/value pairs');
  const values = new Map();
  for (let index = 0; index < argv.length; index += 2) {
    const name = argv[index];
    const value = argv[index + 1];
    if (!OPTIONS.has(name)) throw new TypeError(`Ubuntu capsule release option ${name} is unsupported`);
    if (values.has(name)) throw new TypeError(`Ubuntu capsule release option ${name} is duplicated`);
    values.set(name, value);
  }
  for (const name of OPTIONS) {
    if (name === '--chunk-bytes') continue;
    if (!values.has(name)) throw new TypeError(`Ubuntu capsule release option ${name} is required`);
  }
  for (const name of ['--recipe', '--destination', '--private-key', '--public-key', '--gpgv', '--keyring']) {
    if (!path.isAbsolute(values.get(name)) || values.get(name).includes('\0')) {
      throw new TypeError(`Ubuntu capsule release option ${name} must be an absolute local path`);
    }
  }
...
const [recipeBytes, privateKeyBytes, publicKeyBytes] = await Promise.all([
  readBounded(args.recipe, 'Ubuntu capsule recipe', MAX_RECIPE_BYTES),
...
catch { throw new Error('Ubuntu capsule recipe is not valid JSON'); }
if (!recipe || typeof recipe !== 'object' || Array.isArray(recipe)
    || Object.keys(recipe).some((key) => !['capture', 'artifacts'].includes(key))) {
  throw new TypeError('Ubuntu capsule recipe has unsupported fields');
}
const inReleaseVerifier = new GpgvInReleaseVerifier({ executable: args.gpgv, keyring: args.keyring });
const controller = new AbortController();
const interrupt = (name) => () => controller.abort(new Error(`Ubuntu capsule release interrupted by ${name}`));
...
const input = {
  ...
  privateKeyBytes,
  publicKeyBytes,
  verifyInRelease: (request) => inReleaseVerifier.verify(request),
  signal: controller.signal,
};
if (args.chunkBytes != null) input.chunkBytes = args.chunkBytes;
try {
  const result = await buildUbuntuPackageCapsuleRelease(input);
