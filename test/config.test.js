...
test('uses conservative API, auth, execution, Git, publication, sandbox, and context-rollover defaults', () => {
...
test('sandbox provider and read roots are local, explicit, and bounded', () => {
  const raw = base();
  raw.execution.sandbox = { provider: 'bubblewrap', readRoots: [path.resolve('/opt/toolchain')], verificationTimeoutMs: 30_000 };
  const observed = validateConfig(raw).execution.sandbox;
  assert.equal(observed.provider, 'bubblewrap');
  assert.deepEqual(observed.readRoots, [path.resolve('/opt/toolchain')]);
  assert.equal(observed.verificationTimeoutMs, 30_000);
  raw.execution.sandbox.provider = 'declared-is-enforced';
  assert.throws(() => validateConfig(raw), /provider must be auto, none, bubblewrap, or windows-sandbox/u);
  raw.execution.sandbox.provider = 'bubblewrap';
  raw.execution.sandbox.readRoots = ['relative/toolchain'];
  assert.throws(() => validateConfig(raw), /must be an absolute path/u);
});

test('context rollover policy is local, explicit, and bounded', () => {
  const raw = base();
  raw.contextRollover = { enabled: false, unit: 'proxy', capacityUnits: 20_000, softRatio: 0.4, preferredRatio: 0.6, hardRatio: 0.8, maxHandoffBytes: 65_536, maxRetained: 12 };
  assert.deepEqual(validateConfig(raw).contextRollover, raw.contextRollover);
  raw.contextRollover.softRatio = 0.7; raw.contextRollover.preferredRatio = 0.6;
  assert.throws(() => validateConfig(raw), /softRatio < preferredRatio < hardRatio/u);
  raw.contextRollover.softRatio = 0.4; raw.contextRollover.preferredRatio = 0.6; raw.contextRollover.maxHandoffBytes = 262_145;
  assert.throws(() => validateConfig(raw), /maxHandoffBytes must be <= 262144/u);
  raw.contextRollover.maxHandoffBytes = 65_536; raw.contextRollover.unit = 'guess';
...
test('legacy github.tokenEnv remains first while standard environment fallbacks are added', () => {
  ...
  assert.deepEqual(config.github.auth.environmentVariables, ['UAI_GITHUB_TOKEN', 'PATCH_POLLER_GITHUB_TOKEN', 'GH_TOKEN', 'GITHUB_TOKEN']);
...
test('explicit GitHub auth configuration can constrain the local credential method', () => {
  ...
  assert.deepEqual(config.github.auth, { mode: 'environment', environmentVariables: ['LOCAL_ONLY_GITHUB_TOKEN'], githubCliExecutable: 'gh-custom', hostname: 'github.example.com' });
});

test('GitHub auth rejects unsafe environment-variable names', () => {
  const raw = base(); raw.github.auth = { mode: 'auto', environmentVariables: ['GH_TOKEN;evil'] };
