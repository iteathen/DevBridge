...
test('compiled Windows native sandbox helper can create and remove an AppContainer profile', { timeout: 30_000 }, async (t) => {
  if (process.platform !== 'win32') {
    t.skip('Windows AppContainer probe regression is Windows-only');
    return;
  }

  const root = await mkdtemp(path.join(os.tmpdir(), 'devbridge-appcontainer-probe-'));
  try {
    ...
    const result = await exec(helper, ['--probe'], {
      cwd: root,
      windowsHide: true,
      timeout: 15_000,
    });
    assert.equal(result.stderr, '');
    assert.match(result.stdout, /"provider":"windows-appcontainer"/u);
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});

test('compiled Windows job helper preassigns a detached descendant to kill-on-close containment', { timeout: 30_000 }, async (t) => {
  if (process.platform !== 'win32') {
