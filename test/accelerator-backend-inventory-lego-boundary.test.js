...
test('native Windows substrate adapter is read-only and contains no subsystem or host-command surface', async () => {
  const source = (await readFile(windowsNativeAdapterUrl, 'utf8')).toLowerCase();
  for (const forbidden of ['wsl.exe', '--install', '--update', 'rundll32', 'regsvr32', 'powershell', 'cmd.exe', 'shell: true', 'shell:true']) {
    assert.equal(source.includes(forbidden), false, forbidden);
  }
...
test('Windows WSL substrate adapter contains observation verbs only and never enables shell execution', async () => {
  const source = (await readFile(windowsWslAdapterUrl, 'utf8')).toLowerCase();
  for (const forbidden of ['--install', '--update', '--set-version', '--set-default', '--unregister', '--terminate', 'enable-windowsoptionalfeature', 'disable-windowsoptionalfeature', 'shell: true', 'shell:true']) {
    assert.equal(source.includes(forbidden), false, forbidden);
  }
});
