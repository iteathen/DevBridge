...
test('Windows protected access material persists only a user-scoped protected blob and exact digest', async () => {
  ...
  const calls = [];
  try {
    const material = new WindowsProtectedAccessMaterial({ directory: root, invoke: fakeInvoke(calls), platform: 'win32', entropy: () => FIXED_BYTES });
...
test('Windows protected access material fails closed off Windows and on substituted records', async () => {
  ...
  try {
    const unavailable = new WindowsProtectedAccessMaterial({ directory: root, invoke: fakeInvoke([]), platform: 'linux' });
    await assert.rejects(() => unavailable.ensure(IDENTITY), /unavailable on this host/u);
    assert.throws(() => new WindowsProtectedAccessMaterial({ directory: 'relative', invoke: fakeInvoke([]) }), /directory is invalid/u);
  } finally { await rm(root, { recursive: true, force: true }); }
});
...
test('Windows protected access material completes a real current-user protection round trip', { skip: process.platform !== 'win32' }, async () => {
  ...
  try {
    const material = new WindowsProtectedAccessMaterial({ directory: root, invoke: invokeCommand, entropy: () => FIXED_BYTES });
