...
function snapshot() { return { branch: 'patchpoller/issue-7-bbbbbbbbbbbb', baseSha: '1'.repeat(40), headSha: '2'.repeat(40), dirty: true, changedFiles: ['a.js'], unmergedFiles: [], status: ' M a.js' }; }
...
test('drives multiple tool turns, validates the workspace, and completes without requiring publication', async () => {
  const store = new MemoryStore();
  const calls = [];
  const workspaceManager = { prepareRun: async () => ({ worktreeDir: '/managed/run', branch: snapshot().branch, baseSha: snapshot().baseSha }), snapshot: async () => snapshot(), validate: async () => snapshot(), publishTaskBranch: async () => { throw new Error('should not publish'); } };
  const results = [
...
test('blocked runs consume only trusted matching feedback before resuming', async () => {
  const store = new MemoryStore();
  const workspaceManager = { prepareRun: async () => ({ worktreeDir: '/managed/run', branch: snapshot().branch, baseSha: snapshot().baseSha }), snapshot: async () => snapshot(), validate: async () => snapshot(), publishTaskBranch: async () => ({}) };
  let runs = 0;
