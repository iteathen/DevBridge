...
test('zstd adapter rejects another encoding algorithm before executing', async () => { let called = false; const codec = new ZstdImageCodec({ run: async () => { called = true; } }); await assert.rejects(() => codec.decode({ source: '/a', destination: '/b', algorithm: 'gzip' }), /another encoding/u); assert.equal(called, false); });
