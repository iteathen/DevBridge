...
test('Hyper-V guest delivery binds copy to exact owned environment proof', async () => {
  ...
  try {
    const source = path.join(root, '.seed-random-identity.json');
    ...
    const delivery = new HyperVGuestFileDelivery({
      ...
      invoke: async (input) => {
        supplied = input;
        const native = JSON.parse(input.input);
        assert.equal(path.posix.join(native.destination, path.basename(native.source)), '/var/lib/devbridge/access/seed.json');
        assert.equal(await readFile(native.source, 'utf8'), '{}\n');
        return { exitCode: 0, timedOut: false, aborted: false, outputTruncated: false, stdout: '{"delivered":true}', stderr: '' };
      },
    });
    ...
    assert.equal(payload.destination, '/var/lib/devbridge/access/');
