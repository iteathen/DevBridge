...
test('Ubuntu release source binds signed manifest, exact digest, size, and official host', async () => {
  ...
  try {
    ...
    const payloads = new Map([
      [approved.checksums.manifestUrl, `${approved.media.sha256}  ${approved.media.name}\n`],
    ...
    const source = new UbuntuReleaseMediaSource({
      ...
      verifyManifest: async ({ expectedFingerprint }) => ({ verified: true, signerFingerprint: expectedFingerprint }),
    });
    ...
    assert.match(result.identity.checksumManifestSha256, /^[a-f0-9]{64}$/u);
...
test('Ubuntu release source rejects a media digest not present in the signed manifest', async () => {
  ...
  try {
    ...
    const approved = authority(media);
    const payloads = new Map([
      [approved.checksums.manifestUrl, `${'0'.repeat(64)}  ${approved.media.name}\n`],
      [approved.checksums.signatureUrl, 'signature-bytes'],
    ]);
    ...
    await assert.rejects(() => source.acquire({ authorityRef: 'subject-0123456789abcdef0123456789abcdef', destination: path.join(parent, 'media') }), /signed checksum manifest/u);
...
test('Ubuntu release source refuses non-official URL authority before network effects', async () => {
  ...
  try {
    ...
    const source = new UbuntuReleaseMediaSource({
      ...
      verifyManifest: async () => ({ verified: true, signerFingerprint: approved.checksums.signerFingerprint }),
    });
