...
test('rejects malformed structured output instead of silently accepting it', () => { assert.throws(() => parseToolResult(null, { resultParseError: 'bad json' }), ProtocolError); });
test('rejects non-JSON test entries as protocol data instead of throwing an incidental TypeError', () => { assert.throws(() => parseToolResult({ protocol: 'patch-poller/result-v1', status: 'complete', summary: 'done', tests: [undefined] }), ProtocolError); });
