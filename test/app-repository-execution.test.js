...
import { REPOSITORY_EXECUTION_REQUEST_PROTOCOL, REPOSITORY_SCRATCH_CLEANUP_PROTOCOL } from '../src/runtime/repository-execution.js';
...
function localChannel(root,{onPut=()=>{}}={}){const classes={};for(const name of ['input','work','output','cache','scratch'])classes[name]=path.join(root,name);const ensure=async()=>{for(const dir of Object.values(classes))await mkdir(dir,{recursive:true});};const locate=async(location,{forInput=false}={})=>{await ensure();const candidate=path.join(classes[location.class],...location.path.split('/').filter(x=>x!=='.'));if(!forInput)await mkdir(path.dirname(candidate),{recursive:true});return candidate;};return {
 ...
 async put(_target,source,destination,{maxBytes=32*1024*1024}={}){if(!['input','work','scratch','cache'].includes(destination.class))throw new Error('put class is not host-to-guest');onPut(structuredClone(destination));const file=await locate(destination);let offset=0;const chunks=[];while(true){const part=await source.read({offset,limit:Math.min(16*1024,maxBytes-offset)});const data=Buffer.from(part.data);chunks.push(data);offset+=data.length;if(part.eof)break;if(offset>=maxBytes)throw new Error('put limit');}await writeFile(file,Buffer.concat(chunks));return{bytes:offset,digest:'x'};},
 async get(_target,source,sink,{maxBytes=32*1024*1024}={}){if(!['output','work','scratch','cache'].includes(source.class))throw new Error('get class is not guest-to-host');const file=await locate(source,{forInput:true});const data=await readFile(file);if(data.length>maxBytes)throw new Error('get limit');await sink.write({offset:0,data,eof:true,digest:'x'});return{bytes:data.length,digest:'x'};},
...
test('one persistent environment admits only one active source/candidate session', async () => {
  ...
  try {
    ...
    const execution = await createRepositoryExecution({
      stateDirectory: path.join(temp, 'state'), platform: 'linux',
      routes: { protocol: ENVIRONMENT_EXECUTION_ROUTES_PROTOCOL, routes: [{ subject: '123', profile: 'serial', access: { family: 'linux' } }] },
      rootFor: async () => host, listPaths: async (root) => visible(root), resolveSubject: async () => '123', resolveTool: async (tool) => ({ program: tool, arguments: [] }),
      createState: async () => state, createPreparation: async () => ({ ensure: async () => ({ generation: 'a'.repeat(64) }), connection: async () => ({ family: 'linux' }) }), createChannel: async () => channel,
    });
...
test('representative proposal worker uses only logical transfers through the execution bridge', async () => {
  ...
  try {
    ...
    const execution = await createRepositoryExecution({
      ...
      platform: 'linux',
      routes: { protocol: ENVIRONMENT_EXECUTION_ROUTES_PROTOCOL, routes: [{ subject: '456', profile: 'worker', access: { family: 'linux' } }] },
      rootFor: async () => host,
