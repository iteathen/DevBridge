...
export function validateConfig(raw) {
  ...
  if (!REPOSITORY_RE.test(queueRepository)) {
    throw new ConfigurationError('github.queueRepository must be owner/name');
  }
  ...
  return {
    ...
    state: {
      directory: absolutePath(state.directory ?? '~/.patch-poller/state', 'state.directory')
    },
    execution: {
