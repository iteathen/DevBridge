...
const ALLOWED_KEYS = new Set(['protocol', 'runId', 'taskRevision', 'checkpointId', 'subjectDigest', 'action', 'instructions']);

export function parseDecisionEnvelope(body) {
  if (typeof body !== 'string' || Buffer.byteLength(body, 'utf8') > 48_000) throw new ProtocolError('decision body must be a bounded string');
  const match = body.trim().match(/^```patch-poller-decision[ \t]*\r?\n([\s\S]*?)\r?\n```$/u);
  if (!match) throw new ProtocolError('decision authority comment must contain exactly one standalone patch-poller-decision block');
  ...
  for (const key of Object.keys(value)) if (!ALLOWED_KEYS.has(key)) throw new ProtocolError(`decision field ${key} is not supported`);
