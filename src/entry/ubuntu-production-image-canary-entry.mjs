...
import { createUbuntuProductionImagePhysicalCanary } from '../app/ubuntu-production-image-physical-canary.js';

const MAX_CONFIG_BYTES = 4 * 1024 * 1024;

function parseArguments(argv) {
  if (!Array.isArray(argv) || argv.length !== 3 || !['status', 'run'].includes(argv[0]) || argv[1] !== '--config') {
    throw new Error('usage: ubuntu-production-image-canary-entry.mjs <status|run> --config <absolute-local-config>');
  }
  const location = argv[2];
  if (typeof location !== 'string' || location.length === 0 || location.includes('\0') || !path.isAbsolute(location)) {
    throw new Error('physical canary config path must be absolute');
  }
...
export async function runUbuntuProductionImageCanaryEntry(argv, {
  factory = createUbuntuProductionImagePhysicalCanary,
...
if (invoked) {
  try {
    process.exitCode = await runUbuntuProductionImageCanaryEntry(process.argv.slice(2));
  } catch (error) {
