...
export async function doctor(
  ...
  const effectiveSandboxProvider = sandboxProvider === undefined
    ? createExecutionSandboxProvider(config.execution.sandbox, { env, protectedRoots: [config.state.directory] })
    : sandboxProvider;
  const enforcement = await verifySandboxProvider(effectiveSandboxProvider);
  ...
  for (const [name, raw] of Object.entries(config.tools)) {
    ...
    let available = null;
    if (resolveTools) { await resolveExecutable(profile.executable); available = true; }
    tools.push({ name, available, declaredSandbox: profile.sandbox, enforcement: { ...enforcement }, inputMode: profile.inputMode, layer: 'adapter' });
  }
  if (config.execution.enabled && !config.execution.controllerPlansEnabled && tools.length === 0) throw new Error('execution.enabled is true but neither controller plans nor valid local tool profiles are enabled');
  if (config.execution.defaultTool && !Object.hasOwn(config.tools, config.execution.defaultTool)) throw new Error(`execution.defaultTool does not exist: ${config.execution.defaultTool}`);
  ...
  if (checkGitHubAuth) {
    ...
    if (config.execution.enabled && !githubAuth.available) {
      throw new Error('GitHub authentication is required when execution is enabled. ' + `Checked environment variables: ${githubAuth.environmentVariables.join(', ')}; ` + `GitHub CLI fallback: ${githubAuth.githubCliExecutable} auth token --hostname ${githubAuth.hostname}. ` + 'Set one of the environment variables or authenticate GitHub CLI locally.');
    }
  }
  return {
    ok: true,
    ...
    gitVersion,
    enforcement: {
      requestedPolicy: { repositoryCode: 'verified-sandbox-required', network: 'deny-by-default', outsideProjectWrite: false },
      provider: enforcement,
    },
    capabilities: {
      core: {
        controllerPlans: { enabled: config.execution.controllerPlansEnabled, operations: operationRegistry.describe({ enforcementStatus: enforcement }) },
        toolchains,
        faultInjection,
      },
      adapters: { enabled: config.execution.modelAdaptersEnabled, tools },
    },
    tools,
  };
}
