...
function projectRoot(challenge) {
  return `route-acceptance-${createHash('sha256').update(challenge, 'utf8').digest('hex').slice(0, 16)}`;
}

export function createDeterministicCAcceptanceProposal({ challenge } = {}) {
  const selected = normalizedChallenge(challenge);
  const root = projectRoot(selected);
  return {
    protocol: CONTROLLER_PLAN_PROTOCOL,
    files: [
      { scope: 'ephemeral', action: 'create', path: `${root}/CMakeLists.txt`, content: project(selected) },
      { scope: 'ephemeral', action: 'create', path: `${root}/main.c`, content: source(selected) },
    ],
    operations: [
      { id: 'configure', operation: 'cmake.configure', params: { sourcePath: `${root}/CMakeLists.txt`, buildId: BUILD_ID, buildType: 'Release' } },
...
export function createDeterministicCAcceptancePlan(options) {
  return normalizeControllerPlan(createDeterministicCAcceptanceProposal(options));
}
