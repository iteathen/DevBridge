...
function generatedOperation(command, explicit = null) {
  if (explicit != null) {
    if (typeof explicit !== 'string' || !SAFE_OPERATION.test(explicit)) {
      throw new PolicyError('tool onboarding operation is invalid');
    }
    return explicit;
  }
...
function normalizePolicyEntry(raw, index) {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) {
    throw new PolicyError(`tool onboarding autoIntegrate[${index}] must be an object`);
  }
  for (const key of Object.keys(raw)) {
    if (!['command', 'operation', 'helpArgs'].includes(key)) {
      throw new PolicyError(`tool onboarding autoIntegrate[${index}].${key} is not allowed`);
    }
  }
  if (typeof raw.command !== 'string' || !SAFE_COMMAND.test(raw.command)) {
    throw new PolicyError(`tool onboarding autoIntegrate[${index}].command is invalid`);
  }
  ...
  return Object.freeze({
    ...
    helpArgs: Object.freeze([...helpArgs]),
  });
}

export function validateToolOnboardingPolicy(policy) {
  if (!policy || typeof policy !== 'object' || Array.isArray(policy)) {
    throw new PolicyError('tool onboarding policy must be an object');
  }
  const entries = policy.autoIntegrate ?? [];
  if (!Array.isArray(entries) || entries.length > 32) {
    throw new PolicyError('tool onboarding autoIntegrate must contain at most 32 entries');
  }
