...
const FCOPY_DROP_IN = '/etc/systemd/system/hv-fcopy-daemon.service.d/10-devbridge-uio.conf';
const CAPABILITY = /^[a-z0-9][a-z0-9-]{0,62}-v[1-9][0-9]{0,3}$/u;

const FCOPY_DROP_IN_CONTENT = `[Unit]
ConditionPathExists=
BindsTo=
...
const DEFINITIONS = Object.freeze({
  [HYPERV_FCOPY_UIO]: Object.freeze({
    files: Object.freeze([
      Object.freeze({ path: FCOPY_DROP_IN, content: FCOPY_DROP_IN_CONTENT, permissions: '0644' }),
    ]),
    ...
    qualification: Object.freeze([
      ...
      `printf '%s  %s\\n' '${createHash('sha256').update(FCOPY_DROP_IN_CONTENT).digest('hex')}' '${FCOPY_DROP_IN}' | sha256sum -c - >/dev/null`,
    ]),
  }),
});
