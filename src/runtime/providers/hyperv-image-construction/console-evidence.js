...
function normalizeBytes(raw, expectedBytes) {
  if (raw.length === expectedBytes) return raw;
  if (raw.length === expectedBytes + 4) {
...
export class HyperVConsoleEvidence {
  ...
  async publish(identity, result) {
    ...
    const { width, height, rawBytes } = consoleDimensions(result.width, result.height);
    if (typeof result.imageData !== 'string' || result.imageData.length > Math.ceil((rawBytes + 4) / 3) * 4 || !/^[A-Za-z0-9+/]+={0,2}$/u.test(result.imageData)) {
    ...
    const pixels = normalizeBytes(raw, rawBytes);
    ...
    return Object.freeze({ available: true, capturedAt: captured.toISOString(), location, bytes: bmp.length, sha256, width, height });
  }
}
