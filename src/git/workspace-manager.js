...
async function exists(candidate) {
  try { await stat(candidate); return true; } catch (error) { if (error?.code === 'ENOENT') return false; throw error; }
}
function normalizeRemote(value) { return String(value).replace(/\\/g, '/').replace(/\/$/, ''); }
function safeRunId(runId) {
  if (!/^[A-Za-z0-9_.-]{1,120}$/.test(String(runId))) throw new PolicyError('runId is not safe for a managed path');
  return String(runId);
}
function authBaseUrl(remoteUrl) { try { return `${new URL(remoteUrl).origin}/`; } catch { return null; } }
...
export class GitWorkspaceManager {
  ...
  constructor({ workspacePolicy, gitClient, tokenProvider = async () => null, remoteUrlResolver = (repository) => `https://github.com/${repository}.git`, fetchTimeoutMs = 300_000, branchPrefix = 'patchpoller' }) {
  ...
  branchName(task) { return `${this.#branchPrefix}/issue-${task.issueNumber}-${task.revision.slice(0, 12)}`; }
  ...
  async ensureRepository(repository) {
    ...
    if (!(await exists(path.join(repoDir, '.git')))) {
      if (!this.#workspace.allowCreate) throw new PolicyError(`managed repository does not exist and workspace.allowCreate is false: ${repository}`);
      const parent = path.dirname(repoDir);
      await this.#workspace.assertWriteContained(parent);
      await mkdir(parent, { recursive: true, mode: 0o700 });
      await this.#git.run(['clone', '--no-checkout', '--origin', 'origin', '--', remoteUrl, repoDir], { cwd: parent, token, authBaseUrl: authBaseUrl(remoteUrl), timeoutMs: this.#fetchTimeoutMs });
    }

    const actualRemote = (await this.#git.run(['remote', 'get-url', 'origin'], { cwd: repoDir })).stdout.trim();
    if (normalizeRemote(actualRemote) !== normalizeRemote(remoteUrl)) throw new PolicyError(`managed repository origin mismatch: expected ${remoteUrl}, found ${actualRemote}`);

    await this.#git.run(['fetch', '--prune', '--no-tags', 'origin', '+refs/heads/*:refs/remotes/origin/*'], { cwd: repoDir, token, authBaseUrl: authBaseUrl(remoteUrl), timeoutMs: this.#fetchTimeoutMs });
  ...
  async prepareRun(task, runId) {
    ...
    if (await exists(worktreeDir)) {
      ...
      if (branchExists.exitCode === 0) await this.#git.run(['worktree', 'add', '--', worktreeDir, branch], { cwd: repo.repoDir });
      else await this.#git.run(['worktree', 'add', '-b', branch, '--', worktreeDir, repo.baseRef], { cwd: repo.repoDir });
    }
  ...
  async snapshot(workspace) {
    ...
    return { branch: workspace.branch, baseSha, headSha: head.stdout.trim(), dirty: status.stdout.trim() !== '', changedFiles, unmergedFiles: lines(unmerged.stdout), status: status.stdout.trim() };
  }
  ...
  async validate(workspace) {
    ...
    if (committed.exitCode !== 0 || unstaged.exitCode !== 0) throw new PolicyError(`git diff --check failed: ${(committed.stderr || committed.stdout || unstaged.stderr || unstaged.stdout).trim()}`);
    return snapshot;
  }

  async publishTaskBranch(workspace) {
    if (!workspace.branch.startsWith(`${this.#branchPrefix}/`)) throw new PolicyError('refusing to publish a non-PATCH-POLLER task branch');
    const token = await this.#tokenProvider();
    await this.#git.run(['push', 'origin', `HEAD:refs/heads/${workspace.branch}`], { cwd: workspace.worktreeDir, token, authBaseUrl: authBaseUrl(workspace.remoteUrl), timeoutMs: this.#fetchTimeoutMs });
