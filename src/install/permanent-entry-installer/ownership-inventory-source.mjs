...
function request(raw, expectedIdentity) {
  const value = exactObject(raw, new Set(['identity']), 'ownership inventory observation request');
  if (value.identity !== expectedIdentity) throw new TypeError('ownership inventory observation identity changed');
}

function receiptItem(raw, index) {
  const item = exactObject(raw, new Set(['identity', 'provenance', 'value']), `ownership inventory item ${index}`);
  const selectedIdentity = identity(item.identity, `ownership inventory item ${index}.identity`);
  if (!PROVENANCE.has(item.provenance)) throw new TypeError(`ownership inventory item ${index}.provenance is invalid`);
  if (!item.value || typeof item.value !== 'object' || Array.isArray(item.value)) {
    throw new TypeError(`ownership inventory item ${index}.value is invalid`);
  }
  return Object.freeze({ identity: selectedIdentity, provenance: item.provenance, value: item.value });
}

function ownershipValue(item, valueProtocol, controlIdentity, name) {
...
function receipt(raw, collectionProtocol, valueProtocol, controlIdentity) {
  ...
  if (value.protocol !== collectionProtocol || !GENERATION.test(value.generation)
    ...
    throw new TypeError('ownership inventory receipt is invalid');
  }
  ...
  if (new Set(items.map((member) => member.identity)).size !== items.length) {
    throw new TypeError('ownership inventory receipt contains duplicate identities');
  }
  const control = items.find((member) => member.identity === controlIdentity);
  if (!control || ownershipValue(control, valueProtocol, controlIdentity, 'ownership inventory control').phase !== 'control') {
    throw new Error('ownership inventory control is unavailable');
  }
  return Object.freeze({ generation: value.generation, items: Object.freeze(items) });
}

export function createOwnershipInventorySource({
  ...
  const selectedIdentity = identity(rawIdentity, 'ownership inventory identity');
  if (!collection || typeof collection.read !== 'function') throw new TypeError('ownership inventory collection contract is incomplete');
  if (typeof collectionProtocol !== 'string' || collectionProtocol.length === 0
      || typeof valueProtocol !== 'string' || valueProtocol.length === 0) {
    throw new TypeError('ownership inventory protocol configuration is invalid');
  }
  const selectedControl = identity(controlIdentity, 'ownership inventory control identity');
  if (typeof include !== 'function' || typeof relate !== 'function') {
    throw new TypeError('ownership inventory projection contract is incomplete');
  }
  ...
  return Object.freeze({
    async observe(rawRequest) {
      ...
      const selected = current.items.filter((member) => {
        ...
        const included = include(member.identity);
        if (typeof included !== 'boolean') throw new TypeError('ownership inventory inclusion result is invalid');
        return included;
      }).map((member, index) => Object.freeze({
        ...member,
        value: ownershipValue(member, valueProtocol, selectedControl, `ownership inventory selected item ${index}`),
      }));
      ...
      const items = completeItems.map((member, index) => {
        const selectedRelationships = relationship(
          ...
          `ownership inventory relationship ${index}`,
        );
        return Object.freeze({
